...
= OpenFaaS Babashka template image:{img-ci-tests-status}[link={url-ci-status}] image:{img-ci-hadolint-status}[link={url-ci-status}] image:{img-license}[link=LICENSE] =
...
----
A new project is created for a function defined as `my-bb-function`. It will contain an `fn.handler` namespace that is required for the template to work properly. The requirement for this namespace is to have a top-level function defined as `handler`.
...
See the link:function-examples[function-examples] directory to find a fully working set of OpenFaaS Functions written in Babashka.
