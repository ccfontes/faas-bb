...
- Babashka scripts run immediately, ensuring that Function cold start times are kept to a minimum.
- Babashka is designed for minimal memory usage, allowing you to scale up your Functions without excessively consuming RAM.
- Babashka comes with batteries included, making it easier to write new Functions without having to add the same basic dependencies over and over again.
...
* https://github.com/babashka/babashka#installation[Babashka]: Fast native Clojure scripting runtime.
...
You'll need to install Babashka locally to spin up a REPL environment.
